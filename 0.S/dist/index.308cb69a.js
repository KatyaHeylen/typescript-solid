...
musicToggleElement.addEventListener('click', function() {
    if (car._mediaPlayer.musicLevel === 0) {
        car._mediaPlayer.turnMusicOn();
        musicSliderElement.value = car._mediaPlayer.musicLevel.toString();
        musicToggleElement.innerText = 'Turn music off';
        return;
    }
    musicToggleElement.innerText = 'Turn music on';
    car._mediaPlayer.turnMusicOff();
});
//I use input instead of change, because then the value changes when I move the mouse, not only on release
musicSliderElement.addEventListener('input', function(event) {
    var target = event.target;
    car._mediaPlayer.musicLevel = target.value;
    audioElement.volume = car._mediaPlayer.musicLevel / 100;
    //@todo when you are repeating the same text over and over again maybe we should have made some constants for it? Can you do improve on this?
    musicToggleElement.innerText = car._mediaPlayer.musicLevel ? 'Turn music off' : 'Turn music on';
});
...
setInterval(function() {
    ...
    fuelLevelElement.innerText = car.fuel.toString();
    if (car._mediaPlayer.musicLevel === 0) audioElement.pause();
    else audioElement.play();
}, 1000);

//# sourceMappingURL=index.308cb69a.js.map
